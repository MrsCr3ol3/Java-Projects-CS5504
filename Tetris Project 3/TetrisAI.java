package edu.vt.cs5044;
  ...
  * test the program. It calculates weight cost measure by adding the sum of weight multiplication
  
  * total gaps count, total block count, column height variability count, and maximum block height
  
  * count. It calculates the lowest cost among the column with different shapes and place the shape
  
  * in lowest cost column. purpose, what abstraction it represents, and how to use it.
...
*/
  ...
  public int getColumnHeightVariability(Board board)
  {
      ...
      {
          int colHeightDiff =
              Math.abs(getHeightAtColumn(board, col) - getHeightAtColumn(board, col + 1));
...
/**
 * Cost value will give us a idea about a move. The lower the cost value the better the move is.
 * 
 * @param board
 *            this is the board that's passed
...
public int getTotalWeightedCost(Board board)
{
    ...
    int overallCost =
        totalBlock + colHeightVar + maxBlockHeight + totalGap;
    return overallCost;
}

package adventure;
  ...
  * This class is inherited from the Command class. First the object needs to be taken and then
  
  * dropped in order to shoot.
  ...
  public String execute(Player playerArg)
  {
      ...
      if (room.hasItem(secondWord))
      {
          if (room.hasItem("soccer-ball") && player.isSoccerBallDropped())
          {
              ...
              {
                  return "You shot the soccer ball into the net. "
                      + "You score a goal, " + Message.youWinMessage();
		      }	
		  }

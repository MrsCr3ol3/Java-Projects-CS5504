package adventure;
  ...
  * This class extends the Command class. Taking items will add items to the inventory. 
  
  * This call will remove items from the room.
  ...
  
 {
  ...
  public String execute(Player playerArg)
  {
      ...
      if (room.hasItem(secondWord))
      {
          ...
          if (item.getWeight()
              + player.getInventoryWeight() <= player.getMaxWeight())
          {
              player.addItem(item);
